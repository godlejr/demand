   ...
   <div class="content-body">
      <div class="section-news">
         <div class="news-content" style="padding: 20px; margin-top: 50px;">
            ...
            <form id="news-form" class="news-form" enctype="multipart/form-data" acceptcharset="UTF-8">
               <table>
                  <tbody>
                     <tr>
                        <td class="form-menu">보고자</td>
                        <td class="form-menu-content">${news.user.name}
                           ${news.user.positionCategory.name}님</td>
                     </tr>
                     <tr>
                        <td class="form-menu">제목</td>
                        <td class="form-menu-content"><input type="text" id="title"
                           name="title" class="title-input" placeholder="제목을 입력하세요" value="${news.title }"></td>
                     </tr>
                     ...
                     <tr>
                        ...
                        <td class="form-menu-content">
                           <div id="avatar-file-drop-zone">
                              ...
                              <div class="avatar-file-input"  hidden="hidden">
                                 <label for="avatar-file-input">대표이미지 선택</label> 
                                 <input id="avatar-file-input" class='file' type="file" name="file"
                                    style="cursor: pointer; outline: none;"/>
                              </div>
                              
                              <div class="avatar-file-image" data-id = "${news.avatarFile.id}">
                                 <div class='image-item'> 
                                 <div class='image-delete' onclick="javascript:deleteAvatar(this)">
                                    <i class='fa fa-times-circle' aria-hidden='true'></i>
                                 </div>
                                 <img class='image-preview' id='image-preview' src= '${newsFileUrl}/${news.avatarFile.storageName}'
                                    style='max-width:100%;' onclick="javascript:onClickAvatarImage()">
                                 </div>
                              </div>
                     ...
                     <tr>
                        ...
                        <td>
                           ...
                           <textarea name="smarteditor" id="smarteditor" rows="10"
                              cols="100" style="width: 100%; height: 412px;">${news.content }</textarea>
                     ...
                     <tr>
                        ...
                        <td class="form-menu-content">
                           ...
                           <div class="file-content-zone" ${fn:length(news.newsFiles) gt 0? '':'hidden'}>
                              <ul id="file-list">
                                 <c:forEach var="newsFile" items="${news.newsFiles}">
                                    <li class='list-item'>
                                       <i class='fa fa-file-code-o' aria-hidden='true' onClick='deleteFileList(this," + autoFileCount + ");' ></i>
                                       <span class='item-name'>${newsFile.file.originalName }</span>
                                       <i class='fa fa-times' style='cursor:pointer;'  onClick="javascript:deleteOriginalFileList(this,'${newsFile.file.storageName}')" aria-hidden='true'></i>
                                    </li>
                                 </c:forEach>
                              </ul>
                           </div>
...
<script>
   ...
   var newsFileTempForm = new FormData();
   var newsFileDeleteForm = new FormData();
   ...
   function updateAvatarFileView(data){
      var fileItemHolder = $(".avatar-file-image");
      var fileItemTemplate = "<div class='image-item'> <div class='image-delete' onclick='javascript:deleteAvatar(this)'>"+
   ...
   function deleteAvatarFile(id){
      $.ajax({
         type: "POST",
         url: "${contextPath}/news/${news.id}/avatar/delete",
         data: {fileId: id},
         success: function(data){
            var fileItemHolder = $(".avatar-file-image");
            var fileId = fileItemHolder.attr("data-id");
   ...
   function deleteOriginalFileList(self, originalName){
      --fileSize;
      newsFileDeleteForm.append("deletedFiles",originalName);
      $(self).parent().remove();
   ...
   $submit.click(function(){
      ...
      var deletedFiles = newsFileDeleteForm.getAll("deletedFiles");
      
      for(var i =0 ; i < deletedFiles.length ; i++ ){
         newsForm.append("deletedFileStorageNames", deletedFiles[i]);
      }
   ...
   function navigateToNewsDetail(id){
	   document.location.href = "${contextPath}/news/" + id;
   }
   
   function sendNews(newsForm){
      $.ajax({
         type: "POST",
         url: "${contextPath}/news/${news.id}/edit",
         processData: false, 
         contentType: false,
         enctype: 'multipart/form-data',
         data: newsForm,
